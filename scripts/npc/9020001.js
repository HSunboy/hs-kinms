...
function getPrize(eim,cm) {
	...
	cm.gainItem(4001322, 2);
		if(random <= 2 &&random >= 1){
			cm.gainDY(random2);
		    if(cm.isLeader()){
			cm.worldMessage(6,"玩家：["+cm.getName()+"]带领他的队伍完成了废弃都市组队副本！获得抵用卷："+random2);
				}
			}else if(random >= 3 && random <= 4){
			cm.gainMeso(random1);
			if(cm.isLeader()){
			cm.worldMessage(6,"玩家：["+cm.getName()+"]带领他的队伍完成了废弃都市组队副本！获得金币："+random1);
				}
			}else{
			if(cm.isLeader()){
			cm.worldMessage(6,"玩家：["+cm.getName()+"]带领他的队伍完成了废弃都市组队副本！");
				}
			}

...
public class MapleMapFactory {
                ...
                for (MapleData life : mapData.getChildByPath("life")) {
                    ...
                    if (npcs || !type.equals("n")) {
                        ...
                        if (myLife instanceof MapleMonster) {
                            ...
                            map.addMonsterSpawn(mob, mobTime, team,
    ...
    public static int loadCustomLife() {
        ...
        try {
            ...
            java.sql.PreparedStatement ps = con.prepareStatement("SELECT * FROM `spawns`");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                final int mapid = rs.getInt("mid");
                final AbstractLoadedMapleLife myLife = loadLife(rs.getInt("idd"), rs.getInt("f"), rs.getByte("hide") > 0, rs.getInt("fh"), rs.getInt("cy"), rs.getInt("rx0"), rs.getInt("rx1"), rs.getInt("x"), rs.getInt("y"), rs.getString("type"), rs.getInt("mobtime"));
                if (myLife == null) {
                    continue;
                }

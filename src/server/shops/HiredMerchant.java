...
public class HiredMerchant extends AbstractPlayerStore {
    ...
    public void buy(MapleClient c, int item, short quantity) {
       ...
       /* if (c.getPlayer().getMeso() >= pItem.price * quantity) {
            if (MapleInventoryManipulator.商店防止复制(c, newItem, false)) {
                pItem.bundles -= quantity; // Number remaining in the store
         ...
         if (MapleInventoryManipulator.checkSpace(c, newItem.getItemId(), newItem.getQuantity(), newItem.getOwner())) {
            ...
            if (gainmeso > 0) {
                ...
                log.info(new StringBuilder().append("[雇佣] ").append(chr != null ? chr.getName() : getOwnerName()).append(" 雇佣商店卖出: ").append(newItem.getItemId()).append(" - ").append(itemText).append(" 价格: ").append(theQuantity).toString());
            } else {
                c.getPlayer().dropMessage(1, "金币不足.");
                c.getSession().write(MaplePacketCreator.enableActions());
            }

...
public class MapleInventoryManipulator {
    ...
    public static IItem addbyId_Gachapon(final MapleClient c, final int itemId, short quantity, long period) {
            ...
            if (!GameConstants.isRechargable(itemId)) {
                ...
                while (quantity > 0) {
                    ...
                    if (newQ != 0) {
                        ...
                        if (period > 0) {
                            nItem.setExpiration(System.currentTimeMillis() + (period * 60 * 60 * 1000));
                        }
                        recieved = true;
                        c.getSession().write(MaplePacketCreator.addInventorySlot(type, nItem));
                        if (GameConstants.isRechargable(itemId) && quantity == 0) {

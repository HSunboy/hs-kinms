...
public abstract class AbstractPlayerInteraction {
    ...
    public final void warpPlayer(final int map, final int map2) {
        if (c.getPlayer().getMapId() == map) {
            ...
            c.getPlayer().dropMessage(5, "NPC传送地图函数不匹配！");
        }
    }
...
public final void setPlayerStat(final String type, int x) {
    ...
    public final void gainItem(final int id, final short quantity, final boolean randomStats, final long period, final int slots, final String owner, final MapleClient cg, byte Flag) {
        if (quantity >= 0) {
            ...
            final MapleInventoryType type = GameConstants.getInventoryType(id);//判断物品类型
    ...
    public final void givePartyFb(final int amount) {
        ...
        for (final MaplePartyCharacter chr : getPlayer().getParty().getMembers()) {
            final MapleCharacter curChar = getMap().getCharacterById(chr.getId());
            if (curChar != null) {
            if (curChar.getmrfbrws() > curChar.getFBRW()) {
    ...
    public final void givePartyFba(final int amount) {
        ...
        for (final MaplePartyCharacter chr : getPlayer().getParty().getMembers()) {
            final MapleCharacter curChar = getMap().getCharacterById(chr.getId());
            if (curChar != null) {
            if (curChar.getmrfbrwas() > curChar.getFBRWA()) {
    ...
    public void gainPet(int id, String name, int level, int closeness, int fullness, long period) {//给予宠物
        ...
        try {
            MapleInventoryManipulator.addById(c, id, (short) 1, "", MaplePet.createPet(id, name, level, closeness, fullness, MapleInventoryIdentifier.getInstance(), id == 5000054 ? (int) 1 : 0), period, (byte) 0);
    ...
    public EventInstanceManager getDisconnecteda(String event,int map) {
        ...
        for (EventInstanceManager eim : em.getInstances()) {
            if (eim.isDisconnected(c.getPlayer()) && eim.getPlayerCount() > 0 && getPlayerCount(105100300) <= 0) {
                return eim;
            }
        }
        return null;
    ...
    //火药桶捐献开启双倍判断
    //每个玩家背包里面有火药桶物品捐献给npc以后 会在角色表  gethythd 值会增加
    //然后写代码查询 所有玩家的 角色表里面的  hythd 函数的值。
    //全部加到一起。得出来的总和就行了。
    
    
    //然后在写一个清理所有玩家的 hythd这个表的代码

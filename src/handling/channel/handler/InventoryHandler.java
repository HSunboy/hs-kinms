...
public class InventoryHandler {
    ...
    public static final void UseMountFood(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        c.getPlayer().updateTick(slea.readInt());
        final byte slot = (byte) slea.readShort();
        final int itemid = slea.readInt(); //2260000 usually恢复疲劳补药
        final IItem toUse = chr.getInventory(MapleInventoryType.USE).getItem(slot);
    ...
    public static final void UseTreasureChest(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            final IItem item = MapleInventoryManipulator.addbyId_Gachapon(c, reward, (short) amount, 0);
            ...
            case 5050000: { // AP Reset
                ...
                switch (apfrom) { // AP to
                    ...
                    case 2048: // luk
                        ...
                        break;
                    case 8192: // hp
                         if (playerst.getMaxMp() < ((c.getPlayer().getLevel() * 14) + 134) || c.getPlayer().getHpApUsed() <= 0 || c.getPlayer().getHpApUsed() >= 25000 || playerst.getMaxHp() < 1) {
                      //  if (playerst.getMaxHp() <= 30000) {
                            used = false;
                        }
                        break;
                    case 32768: // mp
                          if (playerst.getMaxMp() < ((c.getPlayer().getLevel() * 14) + 134) || c.getPlayer().getHpApUsed() <= 0 || c.getPlayer().getHpApUsed() >= 25000) {
                        //if (playerst.getMaxMp() <= 30000) {
                            used = false;
                        }
                        break;
...
//                }
            ...
            case 5320000: {
                ...
                c.getSession().write(MTSCSPacket.show塔罗牌(name, otherName, love1, cardId, pcf.getCardCommentSize()));
                used = true;
                break;
            }
            ...
            case 5390024:
            case 5390025:
            case 5390030:{ // Tiger Roar's Messenger
                if (c.getPlayer().getLevel() < 10) {
                    c.getPlayer().dropMessage(5, "必須等級10級以上才可以使用.");
                    break;
                }
            ...
            default:
                if (itemId / 10000 == 512) {
                    ...
                    used = true;
                } else if (itemId == 5201000) {
                    c.getPlayer().gainBeans(2000);
                    c.getSession().write(MaplePacketCreator.updateBeans(c.getPlayer().getId(), itemId));
                    used = true;
                } else if (itemId == 5201001) {
                    ...
                    c.getSession().write(MaplePacketCreator.updateBeans(c.getPlayer().getId(), itemId));
                    used = true;
                } else if (itemId == 5201004) {
                    c.getPlayer().gainBeans(40);
                    c.getSession().write(MaplePacketCreator.updateBeans(c.getPlayer().getId(), itemId));
                    used = true;
                } else if (itemId == 5201005) {
                ...
                } else if (itemId == 5530000) {
                    NPCScriptManager.getInstance().start(c, 9900004, 10086);
                    c.getSession().write(MaplePacketCreator.enableActions());
                    break;
                } else if (itemId / 10000 == 553 && itemId != 5530000) {
    ...
    public static final void HeiLong_BaoWuHe(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        for (RewardItem reward : rewards.getRight()) {
            ...
            if (itemId == 2022613) { //法老王盒子1
              //  NPCScriptManager.getInstance().start(c, 9330008, 1);
                c.getSession().write(MaplePacketCreator.enableActions());
                break;
            }
            if (itemId == 2022618) { //法老王盒子2
              //  NPCScriptManager.getInstance().start(c, 9330008, 2);
                c.getSession().write(MaplePacketCreator.enableActions());
                break;
            }
            if (itemId == 2022336) {
              //  NPCScriptManager.getInstance().start(c, 9020000, 1000); //打开NPC
               // MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, itemId, 1, false, false);
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
            }
            if ((Randomizer.nextInt(rewards.getLeft()) < reward.prob)) {//Is it even possible to get an item with prob 1?USE  //消耗类的黑龙箱子 与 企鹅王的宝物盒的爆率设置

                    ...
                    if (createType == 1 || createType == 2) {
                      ...
                      //  }
                        ...
                        if (createType == 4) {
                          ...
                          /*  byte flag = shop.getFlag();
                            if (ItemFlag.KARMA_EQ.check(flag)) {
                                shop.setFlag((byte) (flag - ItemFlag.KARMA_EQ.getValue()));
                                c.getSession().write(MaplePacketCreator.getCharInfo(chr));
                                chr.getMap().removePlayer(chr);
                                chr.getMap().addPlayer(chr);
                              //  c.getSession().write(MaplePacketCreator.updateSpecialItemUse_(shop, GameConstants.getInventoryType(shop.getItemId()).getType()));
                            } else if (ItemFlag.KARMA_USE.check(flag)) {
                                shop.setFlag((byte) (flag - ItemFlag.KARMA_USE.getValue()));
                                c.getSession().write(MaplePacketCreator.getCharInfo(chr));
                                chr.getMap().removePlayer(chr);
                                chr.getMap().addPlayer(chr);
                               // return;
                        ...
                        /*if (merchant.getItems().size() > 0) {
                            for (MaplePlayerShopItem items : merchant.getItems()) {
                                if (items.bundles > 0) {
                                    ...
                                    if (MapleInventoryManipulator.商店防止复制(c, item_get, false)) {
                                        items.bundles = 0;
                                    } else {
                                        save = true;
                                        break;

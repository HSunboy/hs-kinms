        ...
        if(type == 0){
          ...
          //  slea.readInt();
           ...
           /* } catch (InterruptedException ex) {//捕获该线程异常
            ...
            c.getSession().write(MaplePacketCreator.BeansJDXZ(get进洞次数(), geta(), getb(), getd(), getas(), getbs(), getds()));
            } else {
            c.getSession().write(MaplePacketCreator.BeansJDXZ(get进洞次数(), 0, 0, 0, getas(), getbs(), getds()));
            }
           // }

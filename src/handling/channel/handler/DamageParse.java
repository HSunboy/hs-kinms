...
//        lea.skip(4); //0
        ...
        for (int i = 0; i < ret.targets; i++) {
            ...
            for (int j = 0; j < ret.hits; j++) {
               ...
               /* int skillmad = 0;
                if (ret.skill != 0) {
                    skillmad = SkillFactory.getSkillmad(ret.skill / 10000, ret.skill, chr.getSkillLevel(ret.skill));
                    int supermad = (int) (chr.getStat().getCurrentMaxBaseDamage() * skillmad / 18 + chr.getStat().getCurrentMaxBaseDamage() * skillmad / 10000);
                   //  chr.dropMessage(5, "技能最大伤害值：" + skillmad + " || 实际攻击伤害值：" + damage + " || "+chr.getStat().getCurrentMaxBaseDamage() + " || 技能伤害加成：" + skillmad+"%" + " || mad" + supermad);
                    if (damage > supermad && ret.skill != 2301002) {
                        damage = 1;
                    }
                } else {
                  //  chr.dropMessage(5, "普通攻击最大伤害值：" + chr.getStat().getCurrentMaxBaseDamage() + " || 实际攻击伤害值：" + damage);
                    if (damage > chr.getStat().getCurrentMaxBaseDamage()) {
                        damage = 1;
                    }
                }*/
...
//	    System.out.println(tools.HexTool.toString(lea.read(14)));
            ...
            for (int j = 0; j < ret.hits; j++) {
                ...
                /*int skilldamage = 0;
                if (ret.skill != 0) {
                    skilldamage = SkillFactory.getSkilldamage(ret.skill / 10000, ret.skill, chr.getSkillLevel(ret.skill));
                    int superdamage = (int) (chr.getStat().getCurrentMaxBaseDamage() * skilldamage / 100 + chr.getStat().getCurrentMaxBaseDamage() * skilldamage / 10000);
                    chr.dropMessage(5, "技能最大伤害值：" + superdamage + " || 实际攻击伤害值：" + damage+ " || "+ chr.getStat().getCurrentMaxBaseDamage() + " || 技能伤害加成：" + skilldamage+"%" + " || damage" + superdamage);
                    if (damage > superdamage) {
                        damage = 1;
                    }
                } else {
                    chr.dropMessage(5, "普通攻击最大伤害值：" + chr.getStat().getCurrentMaxBaseDamage() + " || 实际攻击伤害值：" + damage);
                    //c.getPlayer().dropMessage(5, "服务端计算普通攻击最大伤害值：" + c.getPlayer().getCurrentMaxBaseDamage() + " || 当前实际攻击伤害值：" + damage);
                    if (damage > chr.getStat().getCurrentMaxBaseDamage()) {
                        damage = 1;
                    }
                }*/
...
//	    System.out.println(tools.HexTool.toString(lea.read(4)));
    ...
    public static final int Damage_SkillPD(MapleCharacter c, int damage, final AttackInfo ret) {
       if (GameConstants.Novice_Skill(ret.skill)) {//新手技能
            if (damage > 40) {
            ...
            if (GameConstants.Ares_Skill_350(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 13)) {
                    ...
                    damage = 1;
                return damage;
                }//////System.out.println("战神伤害处理A||输出伤害:"+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage/3.6+" ||实际:"+damage+"");
            } else if (GameConstants.Ares_Skill_140(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 20)) {
                    //   c.dropMessage(1,"[战神技能攻击力检测+B]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    damage = 1;
                return damage;
                }//////System.out.println("战神伤害处理B||输出伤害:"+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage/3+" ||实际:"+damage+"");
            } else if (GameConstants.Ares_Skill_1500(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 21)) {
                    //////System.out.println("角色名："+c.getPartner().getName()+"。输出伤害:"+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage/17+"||实际伤害为:"+damage+"");
                    damage = 1;
                return damage;
                    // c.dropMessage(1,"[战神技能攻击力检测+C]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                }
            } else if (GameConstants.Ares_Skill_800(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 14)) {
                    damage = 1;
                return damage;
                    //     c.dropMessage(1,"[战神技能攻击力检测+D]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                }//////System.out.println("战神伤害处理D||输出伤害:"+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage/10+" ||实际:"+damage+"");
                //   c.dropMessage(1,"[战神技能攻击力检测+E]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
            }//////System.out.println("战神伤害处理E||输出伤害:"+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage/2+" ||实际:"+damage+"");
        } else if (c.getJob() == 100 || c.getJob() == 110 || c.getJob() == 111 || c.getJob() == 112
                || c.getJob() == 120 || c.getJob() == 121 || c.getJob() == 122
                || c.getJob() == 130 || c.getJob() == 131 || c.getJob() == 132) { //战士技能
            if (GameConstants.Warrior_Skill_450(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 11)) {
                    //  c.dropMessage(1, "[战士技能攻击力检测+A]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    // //System.out.print(" [检测A] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/7+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
                }
                ...
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 18)) {
                    //     c.dropMessage(1, "[战士技能攻击力检测+B]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    //  //System.out.print(" [检测B] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/14+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
                }
                ...
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 12)) {
                    //    c.dropMessage(1, "[战士技能攻击力检测+C]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    //  //System.out.print(" [检测C] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/10+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;

                }
            } else if (GameConstants.Warrior_Skill_2000(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 24)) {
                    //    c.dropMessage(1, "[战士技能攻击力检测+D]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    //   //System.out.print(" [检测D] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/22+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
            ...
            if (GameConstants.Magician_Skill_90(ret.skill)) {
                ...
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 18)) {
                    ...
                    //    c.dropMessage(1, "[魔法师技能攻击力检测+B]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    //      //System.out.print(" [检测B] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/12+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
                }
                ...
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 20)) {
                    //   c.dropMessage(1, "[魔法师技能攻击力检测+C]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    // //System.out.print(" [检测C] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/5.6+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
                }
                ...
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 36)) {
                    //  c.dropMessage(1, "[魔法师技能攻击力检测+D]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    //    //System.out.print(" [检测D] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/45+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
                }
            ...
            if (GameConstants.Bowman_Skill_180(ret.skill) && (c.getBuffedValue(MapleBuffStat.SHARP_EYES) != null) && (damage > 0)) { //判断火眼
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 13)) {
                    //  c.dropMessage(1, "[弓箭手技能攻击力检测+A]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    //    //System.out.print(" [检测A] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/1.2+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
                }
                ...
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 9)) {
                    //  c.dropMessage(1, "[弓箭手技能攻击力检测+B]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    //  //System.out.print(" [检测B] "+c.getStat().getCurrentMaxBaseDamage()+" <= "+damage+" || "+damage/3.5+"\r\n");
                    damage = 1;
                return damage;
                    //return ret;
                }
            ...
            if (GameConstants.Thief_Skill_180(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 11)) {
                    //    c.dropMessage(1, "[飞侠技能攻击力检测+A]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    damage = 1;
                return damage;
                }
            } else if (GameConstants.Thief_Skill_250(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 14)) {
                    //   c.dropMessage(1, "[飞侠技能攻击力检测+B]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    damage = 1;
                return damage;
                }
            } else if (GameConstants.Thief_Skill_500(ret.skill)) {
                if ((c.getStat().getCurrentMaxBaseDamage() <= damage / 18)) {
                    //      c.dropMessage(1, "[飞侠技能攻击力检测+C]\r\n非法使用外挂或者修改WZ\r\n导致:攻击力过高.\r\n攻击力无效！\r\n请勿再次使用后果自负！");
                    damage = 1;
                return damage;
                }
            } else if (GameConstants.Thief_Skill_800(ret.skill)) {
